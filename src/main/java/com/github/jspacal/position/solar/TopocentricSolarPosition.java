...
public class TopocentricSolarPosition {
    ...
    public TopocentricSolarPosition(
	    SolarPositionAlgorithmSolverSolution solution) {
	this.solution = solution;
    }

    public HorizontalCoordiantes inHorizontalCoordiantes() {
	AngleValue altitude = new AngleValue(
		solution.getTopocentricElevationAngleCorrected(),
		AngleUnit.DEGREE);
	AngleValue zenith = new AngleValue(
		solution.getTopocentricZenithAngle(), AngleUnit.DEGREE);
	Azimuth azimuth = new Azimuth(new AngleValue(
		solution.getTopocentricAzimuthAngleWestwardFromSouth(),
		AngleUnit.DEGREE), new AngleValue(
		solution.getTopocentricAzimuthAngleEastwardFromNorth(),
		AngleUnit.DEGREE));
	...
	AngleValue rightAscension = new AngleValue(
		solution.getTopocentricSunRightAscension(), AngleUnit.DEGREE);
	AngleValue hourAngle = new AngleValue(
		solution.getTopocentricLocalHourAngle(), AngleUnit.DEGREE);
	AngleValue declination = new AngleValue(
		solution.getTopocentricSunDeclination(), AngleUnit.DEGREE);
